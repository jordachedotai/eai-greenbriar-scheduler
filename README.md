...
Phase 5 complete through batch 3: six stages ending in Send invites, real Greenbriar companies and deal teams, faces and logos, rows, board, and company page per `reference/design`, one structured email renderer with the EA's sign-off, reply emails behind every pick and confirmation with a drawer, options inline and one attachment, venues the tool learns as cards, an editable step 1 shortlist with the one-pager following, no native selects, People, Settings, Templates. Rehearsed per `docs/DEMO_SCRIPT.md`. Pending Peggy: EA ownership and the OnTrac and Towne cities. See `BUILD_PLAN.md`.
